  ...
  return (
    <Link
      ...
      className={`CartButton ${
        props.type === "desktop" ? `CartButton--desktop` : "CartButton--mobile"
      }`}
    >
      <svg
      ...
      <span
        className={`CartButton-amount    ${
          cartServices.length >= 1 ? "CartButton-amount--filled" : ""
        }`}
      >
        {`${cartServices.length} ${
          cartServices.length === 1 ? "task" : "tasks"
        }`}
      </span>
    </Link>
  )
